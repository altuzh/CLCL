...
$objects = Get-ChildItem "$repo\Release\*.obj" | Where-Object { $_.Name -ne 'main.obj' -and $_.Name -notlike '*_test.obj' } | ForEach-Object FullName
$compiler = "$($vc.FullName)\bin\Hostx64\x86\cl.exe"
& $compiler /nologo /W3 /WX /O2 /MT /Gy /DWIN32 /D_WINDOWS /DOP_XP_STYLE /DLC_JP /DUNICODE /D_UNICODE /I. tests\menu_regression.c /FoRelease\menu_regression_test.obj /FeRelease\menu_regression_test.exe $objects /link /SUBSYSTEM:CONSOLE /OPT:REF user32.lib gdi32.lib comctl32.lib imm32.lib winsqlite3.lib shell32.lib ole32.lib oleaut32.lib uuid.lib advapi32.lib comdlg32.lib
